...
def build_vless_outbound(raw: dict, index: int):
    """VLESS shape tuned for sing-box 1.11.x (minimal utls; omit empty flow)."""
...
def main():
    ap = argparse.ArgumentParser(
        description="sing-box 1.11.x legacy config templating HTTP server (same query API as the default generator)"
    )
    ap.add_argument(
        "--file",
        required=True,
        help="Path to base sing-box 1.11.x config (JSON) with {{placeholders}}",
    )
