{ config, lib, ... }: {
  ...
  config = {
    services.restic.backups = lib.mkIf config.backup.enable (lib.mapAttrs
      (name: cfg: {
        initialize = true;
        ...
        inherit (cfg)
          paths dynamicFilesFrom backupPrepareCommand backupCleanupCommand;
      }) config.backup.backups);

    age.secrets = lib.mkIf config.backup.enable {
      restic-key = { file = ./secrets/restic-key.age; };
