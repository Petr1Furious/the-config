let
  ...
  potato_server =
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFu0hmSgRB/X46yemDNfwldqQKWU1fkorCI94qokIpUW";
  vm_key =
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIH6tQWk4OUIreAOhMy/UH7CmtfHFcLGZNyPOhjVkwd6i";
in {
  "restic-key.age".publicKeys = [ admin potato_server vm_key ];
  "rclone-config.age".publicKeys = [ admin potato_server vm_key ];
  "vaultwarden-admin-token.age".publicKeys = [ admin potato_server vm_key ];
  "tgauth-key.age".publicKeys = [ admin potato_server vm_key ];
  "tgauth-env.age".publicKeys = [ admin potato_server vm_key ];
  "traefik-env.age".publicKeys = [ admin potato_server vm_key ];
}
